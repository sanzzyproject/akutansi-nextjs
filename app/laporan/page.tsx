...
export default function TransaksiPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // LOGIKA BARU: Validasi Keseimbangan Akuntansi
    const totalHarta = (form.kas || 0) + (form.perlengkapan || 0) + (form.peralatan || 0);
    const totalPasiva = (form.utangUsaha || 0) + (form.modal || 0);

    if (totalHarta !== totalPasiva) {
      toast({ 
        variant: "destructive", 
        title: "Transaksi Tidak Seimbang!", 
        description: `Total Harta (${totalHarta}) tidak sama dengan Utang + Modal (${totalPasiva}). Cek kembali input plus (+) dan minus (-) nya.` 
      });
      return; // Stop fungsi, transaksi batal disimpan jika tidak balance
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input placeholder="Contoh: Beli perlengkapan" name="keterangan" value={form.keterangan} onChange={handleChange} required />
              </div>
            </div>
